...
桌面和平板索引采用扣除导航高度的一屏布局：左上标题与筛选不滚动，左下项目列表为可键盘聚焦的独立滚动区域，右侧预览保持原位。常规高度一屏显示四项；极矮窗口保留最小可用高度，允许页面滚动以避免遮挡。640px 以下恢复自然滚动，项目按钮在上，图片预览在下。
...
列表采用左侧项目索引、右侧单图预览，不再使用通栏精选图或双列大卡片。所有设备点击标题按钮（键盘 Enter / Space）只切换图片和项目说明；独立的「查看项目」链接进入当前项目详情。标题使用 aria-pressed 标记选中状态，不再依赖悬停或聚焦切换。预览保持位置，完整显示图片；减少动态效果偏好下取消淡入淡出。英文示例名称及封面集中在数据文件，明确标记为占位，不代表实际合作。

本地开发模式的 React 调试脚本会被当前 CSP 拦截，交互验收使用 `npm run build` 后的 `npm run start -- --port 3215`。本次不放宽 CSP；生产模式预览不支持编辑后热更新，改动后需重新构建。
