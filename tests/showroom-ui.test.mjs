...
test('cover runs a horizontal gallery that pauses for scroll-controlled movement', async () => {
  ...
  assert.match(experience, /elapsed \* 0\.052/)
  ...
  assert.match(css, /\.showroom-cover__stage\s*\{[^}]*position: sticky/)
  assert.match(css, /\.showroom-cover__track\s*\{[^}]*display: flex/)
  assert.match(css, /\.showroom-cover__lettering \.showroom-cover__word\s*\{[^}]*font-family: "Avenir Next"[^}]*font-weight: 300[^}]*letter-spacing: 0\.01em/)
