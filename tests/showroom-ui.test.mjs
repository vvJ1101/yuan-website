...
test('cover runs a horizontal gallery that pauses for scroll-controlled movement', async () => {
  ...
  assert.match(experience, /zoom=14/)
  assert.match(experience, /enableHighAccuracy: true/)
  assert.match(experience, /maximumAge: 0/)
  assert.match(experience, /address\.city_district \|\| address\.district \|\| address\.borough \|\| address\.county/)
  assert.match(experience, /displayParts\.find\(\(part\) => \/市\$\//)
