...
test('cover uses editorial image fragments and split transparent lettering', async () => {
  ...
  assert.match(experience, /showroom-cover__sticky-lettering/)
  ...
  assert.match(experience, /'YUAN'\.split\(''\)\.map/)
  assert.match(experience, /'SHOWROOM'\.split\(''\)\.map/)
  assert.equal((experience.match(/data-home-scene/g) ?? []).length, 3)
  assert.match(experience, /IntersectionObserver/)
  assert.match(experience, /showroom-cover__letter-layer--base/)
  assert.match(experience, /showroom-cover__letter-layer--image/)
  assert.match(css, /\.showroom-cover__letter-layer--base\s*\{[^}]*color: rgb\(8 8 8 \/ 78%\)/)
  assert.match(css, /\.showroom-cover__sticky-lettering\s*\{[^}]*position: sticky/)
  assert.match(css, /\.showroom-cover__scene\s*\{[^}]*min-height: calc\(100svh - var\(--ys-header-h\)\)/)
  assert.match(css, /\.showroom-cover__sticky-lettering\[data-active-scene='2'\]/)
  assert.match(css, /\.showroom-cover__letter-layer--image\s*\{[^}]*color: rgb\(255 255 255 \/ 82%\)/)
