...
test('cover runs a horizontal gallery that pauses for scroll-controlled movement', async () => {
  ...
  assert.match(experience, /elapsed \* \(pointerSlowingRef\.current \? 0\.024 : 0\.095\)/)
  ...
  assert.match(experience, /nominatim\.openstreetmap\.org\/reverse/)
  ...
  assert.match(experience, /31\.2304° N/)
  assert.match(experience, /121\.4737° E/)
  ...
  assert.match(css, /\.showroom-cover__lettering \.showroom-cover__word\s*\{[^}]*font-family: "Avenir Next"[^}]*font-weight: 400[^}]*letter-spacing: 0\.005em/)
